...
<head>
    <meta charset="UTF-8">
    <title>طباعة المشاركين حسب المحافظة</title>
    <style>
        body { font-family: Arial, sans-serif; direction: rtl; margin: 10px; font-size: 12px; }
        .city-section { margin-bottom: 20px; page-break-after: always; }
        .city-section:last-child { page-break-after: auto; }
        .city-header { font-size: 20px; font-weight: bold; margin-bottom: 15px; text-align: center; background: #667eea; color: white; padding: 10px; }
        .date-section { margin-bottom: 15px; }
        .date-header { font-size: 16px; font-weight: bold; margin-bottom: 5px; background: #f0f0f0; padding: 5px; }
            ...
            @foreach($dates as $date)
                ...
                @if($totalCount > 0)
                    <div class="date-section">
                        <div class="date-header">يوم {{ $date }} - الإجمالي: {{ $totalCount }}</div>
